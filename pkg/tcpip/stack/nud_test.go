...
func (*fakeRand) Float32() float32 { return 0.5 }
...
func TestNUDStateReachableTime(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantMin:           time.Duration(math.MaxInt64 / 1000),
			wantMax:           time.Duration(math.MaxInt64 / 500),
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			s := stack.NewNUDState(c, &fakeRand{})
...
func TestNUDStateRecomputeReachableTime(t *testing.T) {
	...
	const defaultMin = 2 * defaultMaxRandomFactor
	const defaultMax = 3 * defaultMaxRandomFactor
