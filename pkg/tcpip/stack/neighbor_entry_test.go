...
func (c *fakeClock) AfterFunc(l sync.Locker, d time.Duration, f func()) context.CancelFunc {
	...
	sort.Slice(c.mu.work, func(i, j int) bool {
		return c.mu.work[i].executeOn.Before(c.mu.work[j].executeOn)
	})
	...
	return func() {
		...
		for i, item := range c.mu.work {
			if item.executeOn.Equal(executeOn) {
...
// nextWorkItems consumes the next batch of work items with the same scheduled
// execution time. Returns with an empty work item and false if there is no
// work left.
func (c *fakeClock) nextWorkItems() ([]workItem, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	var items []workItem
	executeOn := c.mu.work[0].executeOn

	for {
		var item workItem
		item, c.mu.work = c.mu.work[0], c.mu.work[1:]
		items = append(items, item)
		if len(c.mu.work) == 0 || c.mu.work[0].executeOn != executeOn {
			break
		}
	}
