...
func TestDirectRequest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Verify an ARP response was sent. There is no need to perform a
			// blocking read here, since ARP responses are sent in the same function
			// that handles ARP requests.
			pi, _ := c.linkEP.Read()
